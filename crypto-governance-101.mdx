...
- `_moveVotingPower` during transfers  
- `_writeCheckpoint` during mint/burn

**Symptom:** Proposal results don’t match expectations or delegates appear to have 0 voting power.
...
If your token has custom transfer logic or restricted transfers, ensure voting checkpoints remain correct.
...
- Skipping `_writeCheckpoint` on burns

This leads to incorrect quorum calculations.
...
Some protocols assume multi-hop propagation, which is incorrect.
