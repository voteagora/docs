...
For detailed information about roles and permissions, see the [Governor Roles documentation](/agora-governor/agora-governor-roles).
...
```mermaid
graph TD
    A[Voting cycle begins] --> B[Draft proposal on forum]
    B --> C[Manager]
    C[Manager] --> D[Governor Voting]
    D --> E[Timelock]
    D --> F[Defeated]
    E --> G[Token transfers or Governor changes]
    ...
    style F fill:#ffeded,stroke:#333,stroke-width:2px
